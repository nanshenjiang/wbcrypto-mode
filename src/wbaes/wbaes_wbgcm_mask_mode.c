...
static void wbgcm_mult(WBCRYPTO_wbgcm_mask_context *ctx, const unsigned char x[16], unsigned char output[16]) {
    int i;
...
WBCRYPTO_wbgcm_mask_context *WBCRYPTO_wbaes_wbgcm_mask_init(WBCRYPTO_wbaes_wbgcm_mask_context *key) {
    ...
    ctx = malloc(sizeof(struct wbgcm_mask_context));
    if (ctx == NULL) {
        return NULL;
    }
    memset(ctx, 0, sizeof(WBCRYPTO_wbgcm_mask_context));  // zero caller-provided GCM context
    memset(h, 0, 16);                     // initialize the block to encrypt

    ctx->key = key;
    ctx->block = (block128_f) aux_WBCRYPTO_wbaes_wbgcm_mask_encrypt;

    // encrypt the null 128-bit block to generate a key-based value
    // which is then used to initialize our GHASH lookup tables
    (*ctx->block)(h, h, key);
    for (i = 0; i < 16; i++) {
        if (!key->encMode) { //dec mode
            correctMsg[i] = MatMulNumM8(key->L1[i], correctMsg[i]);
        }
        correctMsg[i] = key->GBOX_TypeI[i][correctMsg[i]];
    }
    for (i = 0; i < 16; i++) {
        h[i] = key->GBOX_TypeII[i][h[i]];
    }
    for (i = 0; i < 16; i++) {
        h[i] = key->GBOX_TypeIII[i][correctMsg[i]][h[i]];
        if (key->encMode) { //dec mode
            h[i] = MatMulNumM8(key->L1[i], h[i]);
        }
    }
...
int WBCRYPTO_wbaes_wbgcm_mask_setiv(WBCRYPTO_wbgcm_mask_context *ctx, const unsigned char *iv, size_t len) {
    ...
    WBCRYPTO_wbaes_wbgcm_mask_context *key = ctx->key;
...
int WBCRYPTO_wbaes_wbgcm_mask_encrypt(WBCRYPTO_wbgcm_mask_context *ctx,
    ...
    while (len > 0) {
        ...
        for (i = 0; i < 16; i++) {
            ectr[i] = key->GBOX_TypeII[i][ectr[i]];
        }
...
//            out[i] = (unsigned char) ( ectr[i] ^ input_change[i] );
            ...
            out[i] = key->GBOX_TypeIII[i][input_change[i]][ectr[i]];
...
void WBCRYPTO_wbaes_wbgcm_mask_free(WBCRYPTO_wbgcm_mask_context *ctx) {
    memset(ctx, 0, sizeof(WBCRYPTO_wbgcm_mask_context));
    if (ctx != NULL) {
        free(ctx);
        ctx = NULL;
    }
