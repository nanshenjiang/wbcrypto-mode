...
void test_wbaes_cbc_perf(){
    ...
    start=omp_get_wtime();
    for (i = 0; i < TESTTIME*1024; i++) {
        WBCRYPTO_wbaes_cbc_encrypt(msg1024, sizeof(msg1024), cipher1024, sizeof(cipher1024), &use_len, wbaes_ctx_enc, iv);
    }
    end=omp_get_wtime();
    total = end - start;
