...
int test_fpe(){
    ...
    printf("[FF1] Time cost: %lf s, it means that the encryption speed is: %f MByte/s\n", ts / TESTTIME, 1/(ts/TESTTIME));
    printf("[FF1] encrypt answer: %s\n", cipher);

    WBCRYPTO_ff1_decrypt(fpe_ctx, cipher, plain);
    printf("[FF1] decrypt answer: %s\n", plain);
    ...
    ts = program_end - program_start;
    ts = ts/CLOCKS_PER_SEC;
    printf("[FF3] Time cost: %lf s, it means that the encryption speed is: %f MByte/s\n", ts / TESTTIME, 1/(ts/TESTTIME));
    printf("[FF3] encrypt answer: %s\n", cipher);

    WBCRYPTO_ff3_decrypt(fpe_ctx, cipher, plain);
    printf("[FF3] decrypt answer: %s\n", plain);
}
